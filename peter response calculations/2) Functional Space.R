# Run after 1) Transect_Response_Variables
#-------------------------------------------------------------
...
###############################################################################
# FIGURE – Functional Trait Space of Reef Fish Assemblages (Thesis Version)
...
fish_assemblage <- EmobsCombined_Filled %>%
  ...
  left_join(opcode_groups %>% select(OpCode, Site, 'Fishing Grounds'),
            by = "OpCode")
...
for (i in seq_along(titles)) {
  ...
  site_name <- titles[i]
  site_fg <- unique(fish_assemblage$'Fishing Grounds'[fish_assemblage$Site == site_name])
...
pred_long <- predictors %>%
  select(Site, 'Fishing Grounds', TOP, TEve, TDiv,
         FC, HD, HM, IM, IS, OM, PK) %>%  # adjust to your actual trait/diet codes present
  pivot_longer(cols = -c(Site, 'Fishing Grounds'),
               names_to = "predictor",
               values_to = "value")
