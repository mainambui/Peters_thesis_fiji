...
# Sum-to-zero contrasts for valid Type III tests
options(contrasts = c("contr.sum", "contr.poly"))

#one-time init for model chooser table
...
analyze_reef_data_enhanced <- function(
  ...
  for (resp in response_vars) if (resp %in% names(analysis_data)) {
    cat("\n--- LM for:", resp, "---\n")
    fixed_terms_lm <- fixed_screened  #keep the same screened set as LMM
