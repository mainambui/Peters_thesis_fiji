...
load_reef_data <- function(file_path = "Fish_Data_Fiji_Final_LF_Dez2024.csv") {
...
data <- load_reef_data()
data$logBiomass <- log(data$biomass_kg_ha)
data$logabun <- log1p(data$abundance_ind_250m2)
data$Cnetflow <- (data$crypto5BROF - data$crypto5BRIF)/(data$crypto5BROF  + data$crypto5BRIF)
data$Rnetflow <- (data$resid15BROF - data$resid15BRIF)/(data$resid15BROF  + data$resid15BRIF)
...
clean_data <- data %>%
  ...
  filter(!if_any(any_of(c("logBiomass","logabun","fric","fdiv","fspe","fori","feve","fdis")), is.na)) %>%
...
run_univariate_panel <- function(analysis_data,
                                 ...
                                 random_effects = c("(1|Fishing_Ground)"),
  ...
  for (resp in response_vars) {
    ...
    for (pr in preds) {
      ...
      if ("Fishing_Ground" %in% names(df) && length(unique(df$Fishing_Ground)) > 1) {
        f_lmm <- stats::as.formula(paste(resp, "~", pr, "+ (1|Fishing_Ground)"))
        fit_lmm <- tryCatch(
...
analysis_config <- list(
  core_ecological = list(
    response_vars = c("logBiomass","logabun","fric","feve"),
    predictor_vars = c(
     "sedimnt","nutrint","NO_TK_AREA","Fishing_Ground","grav_NC","Median_Depth",
       "crypto5LR", "resid15LR","resid15BRIF","pare5BRin","Rnetflow","Cnetflow","transi15LR",
     "transi15BRIF",
  ...
  functional_diversity = list(
    response_vars = c("fric","fdiv","fspe","fori","feve","fdis"),
    predictor_vars = c(
      "sedimnt","nutrint","NO_TK_AREA","Fishing_Ground","grav_NC","Median_Depth",
      "crypto5LR", "resid15LR","resid15BRIF","pare5BRin","Rnetflow","Cnetflow","transi15LR",
      "transi15BRIF",
  ...
  biomass_focused = list(
    response_vars = c("logBiomass","logabun"),
    predictor_vars = c(
      "sedimnt","nutrint","NO_TK_AREA","Fishing_Ground","grav_NC","Median_Depth",
      "crypto5LR", "resid15LR","resid15BRIF","pare5BRin","Rnetflow","Cnetflow","transi15LR",
      "transi15BRIF",
  ...
  all_responses = list(
    response_vars = c("logBiomass","logabun","fric","fdiv","fspe","fori","feve","fdis"),
    predictor_vars = c(
      "sedimnt","nutrint","NO_TK_AREA","Fishing_Ground","grav_NC","Median_Depth",
      "crypto5LR", "resid15LR","resid15BRIF","pare5BRin","Rnetflow","Cnetflow","transi15LR",
      "transi15BRIF",
...
analyze_reef_data_enhanced <- function(
  ...
  for (resp in response_vars) if (resp %in% names(analysis_data)) {
    ...
    mixed_model <- tryCatch(
      ...
      error = function(e) {
        cat("Complex LMM failed, trying simpler structure with (1|Fishing_Ground):\n")
        simple_formula <- as.formula(paste(resp, "~", fixed_terms, "+ (1|Fishing_Ground)"))
        lmerTest::lmer(
          simple_formula,
          data = analysis_data,
          control = lme4::lmerControl(optimizer = "bobyqa")
