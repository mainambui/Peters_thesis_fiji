...
write.csv(EmobsCombined, "EmobsCombined.csv", row.names = FALSE)
...
write.csv(species_list, "EC_Species_List.csv", row.names = FALSE)

# Get family list from dataset
family_list <- unique(str_trim(sort(EmobsCombined$Family)))
write.csv(family_list, "EC_Family_List.csv", row.names = FALSE)
...
write.csv(EmobsFiltered, "EmobsFiltered.csv", row.names = FALSE)

# Identify filtered-out rows
FilteredOut <- anti_join(EmobsCombined, EmobsFiltered)
# 13 rows are filtered out
write.csv(FilteredOut, "EmobsFiltered_List.csv", row.names = FALSE)
...
common_species <- intersect(rownames(traits_mat_unique), colnames(abund_matrix))
